...
%           map = biograph(matrix,ids,'ShowTextInNodes','ID','LayoutType', 'hierarchical') %create a biograph object EdgeType/ 'LayoutType', 'evuilibrium'
            map = digraph(matrix,ids) %create a biograph object EdgeType/ 'LayoutType', 'evuilibrium'
            ...
            h=plot(map,'Layout','force','NodeColor','white'); %generate layout
            ...
            for p=length(MetsNodesIDs)+1:length(ids) %cycle through the nodes IDs starting from the first reaction node
                ...
                if flux(RxIDs(p-length(MetsNodesIDs)))>1e-9 && flux(RxIDs(p-length(MetsNodesIDs)))~='x' %if flux is positive and not equal to x                    
                    %[edges1,edges2] = findedge(map,ids(p)); %get all source edges connected to particular reaction node 
                    edges2 = inedges(map,ids(p));
                    edges1 = outedges(map,ids(p));  %get all sink edges connected to particular reaction node  
                    tot_edges=[edges1;edges2]; %concatenate edges
                    highlight(h,ids(p),'NodeColor','blue')
                    highlight(h,'edges',tot_edges,'EdgeColor','g','LineWidth',abs(7*(flux(RxIDs(p-length(MetsNodesIDs))))/max_flux));
                    %highlight(h,edges1,'EdgeColor','g','LineWidth',abs(7*(flux(RxIDs(p-length(MetsNodesIDs))))/max_flux));
                    %set(tot_edges,'EdgeColor',[1 0 0]); %set green color to found edges
                    %set(tot_edges,'LineWidth',abs(7*(flux(RxIDs(p-length(MetsNodesIDs))))/max_flux)); %set thichness of edge proportional to max flux 
                elseif flux(RxIDs(p-length(MetsNodesIDs)))<-1e-9 && flux(RxIDs(p-length(MetsNodesIDs)))~='x' %if flux is negative and not equal to x 
                    %[edges1,edges2] = findedge(map,ids(p)); %get all source edges connected to particular reaction node
                    highlight(h,ids(p),'NodeColor','blue')
                    edges2 = inedges(map,ids(p));
                    edges1 = outedges(map,ids(p)); %get all sink edges connected to particular reaction node                 
                    tot_edges=[edges1;edges2]; %concatenate edges 
                    highlight(h,'edges',tot_edges,'EdgeColor','r','LineWidth',abs(7*(flux(RxIDs(p-length(MetsNodesIDs))))/max_flux));
                    %highlight(h,'edges',edges2,'EdgeColor','r','LineWidth',abs(7*(flux(RxIDs(p-length(MetsNodesIDs))))/max_flux));
                    %set(tot_edges,'EdgeColor',[0 1 0]); %set green color to found edges
                    %set(tot_edges,'LineWidth',abs(7*(flux(RxIDs(p-length(MetsNodesIDs))))/max_flux)); %set thichness of edge proportional to max flux               
                else
                    edges2 = inedges(map,ids(p));
                    edges1 = outedges(map,ids(p)); %get all sink edges connected to particular reaction node                 
                    tot_edges=[edges1;edges2]; %concatenate edges
                    highlight(h,'edges',tot_edges,'EdgeColor','white');
               ...
               highlight(h,RxnsNodesIDs,'Marker','square','NodeColor',[241/255 176/255 102/255]); %set color to reaction nodes
               highlight(h,MetsNodesIDs,'Marker','o','NodeColor',[0 0.4470 0.7410]); %set shape of ellipse to metabolite nodes
            ...
            if DeadMetsID(1)~=-1 %if dead end mets exist
                highlight(h,DeadMetsID,'NodeColor',[1 0 0]); %set red color to dead end metabolite nodes
            end           
            ...
            if ExRxnsID(1)~=-1 %if exchange reactions exist
                highlight(h,ExRxnsID,'NodeColor',[128/255 0 128/255]); %set red NodeColor to exchange reaction nodes                
            end
   
            if nargin>5 && ~strcmp(initialMet{1},'') %if the number of arguments > 5 and the argument initialMet is not empty
                highlight(h,initialMetID,'NodeColor',[1 165/255 0]); %set green NodeColor to initial met node 
            end            
        
            if DeadRxnsID(1)~=-1 %if dead reactions exist
                highlight(h,DeadRxnsID,'NodeColor',[1 0 0]); %set red NodeColor to dead reaction nodes
            end
            
            %if ExRxnsID(1)~=-1 %if exchange reactions exist                
            %    highlight(h,id,'Shape','diamond'); %set shape of ellipse to metabolite nodes
            %end
